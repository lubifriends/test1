...
class GoogleTrendingNowCollector:
    """Download South Korea's current Google Trending Now RSS export."""
            ...
            records.append(
                {
                    ...
                    "source": "google_trending_now_rss",
                }
            )
        if not records:
...
def main() -> None:
    ...
    try:
        ...
        log_collection_run(args.db, "google", "success", len(rows), started_at=started)
        print(f"Collected {len(rows)} Google Trending Now records.")
    except Exception as exc:
        log_collection_run(args.db, "google", "failed", 0, str(exc), started_at=started)
        raise
